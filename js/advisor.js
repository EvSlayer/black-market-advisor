...
function analyze(data){
  ...
  const commodityOptions = data.commodities.map(c=>{
    ...
    return {
      ...
      price:c.price,
      history:hist,
      recordLow:Number.isFinite(Number(c.recordLow))?Number(c.recordLow):Math.min(...hist,c.price),
      recordHigh:Number.isFinite(Number(c.recordHigh))?Number(c.recordHigh):Math.max(...hist,c.price),
      recordLowAt:c.recordLowAt||null,
      recordHighAt:c.recordHighAt||null,
      target,
      min,
      avg,
      baseBuyThreshold,
